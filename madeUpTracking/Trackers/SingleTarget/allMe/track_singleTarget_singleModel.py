    ...
    def feedMeasurement(self, measurement, dt):
        ...
        if(self.track is None and len(self.measurements) > 1):
            ...
            if(self.modelType == 0):
                ...
                self.track.S = np.dot(H, np.dot(self.track.P_predict, H.T)) + MeasurementNoiseCovs[0]
                kalmanGain = np.dot(self.track.P_predict, np.dot(H.T, np.linalg.inv(self.track.S)))
                ...
                sigmaPoints = uH.generateSigmaPoints(self.track.x, self.track.P, self.lambda_)
                self.track.x_predict, self.track.P_predict, self.track.S, predictedMeasureMean, kalmanGain = uH.calculatePredictedState(f_predict_model1, dt, h_measure_model1, sigmaPoints, self.Ws, self.Wc, ProcessNoiseCovs[1], MeasurementNoiseCovs[1])

                #update
                diff = measurement - predictedMeasureMean
                self.track.x = self.track.x_predict + np.dot(kalmanGain, diff)
                self.track.P = self.track.P_predict - np.dot(kalmanGain, np.dot(self.track.S, kalmanGain.T))
                self.track.z = h_measure_model1(self.track.x)
                ...
                #generate sigma points and then predict
                sigmaPoints = uH.generateSigmaPoints(self.track.x, self.track.P, self.lambda_)
                self.track.x_predict, self.track.P_predict, self.track.S, predictedMeasureMean, kalmanGain = uH.calculatePredictedState(f_predict_model2, dt, h_measure_model2, sigmaPoints, self.Ws, self.Wc, ProcessNoiseCovs[2], MeasurementNoiseCovs[2])
                ...
                #generate sigma points and then predict
                sigmaPoints = uH.generateSigmaPoints(self.track.x, self.track.P, self.lambda_)
                self.track.x_predict, self.track.P_predict, self.track.S, predictedMeasureMean, kalmanGain = uH.calculatePredictedState(f_predict_model3, dt, h_measure_model3, sigmaPoints, self.Ws, self.Wc, ProcessNoiseCovs[3], MeasurementNoiseCovs[3])
